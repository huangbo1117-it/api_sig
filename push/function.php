...
function category_compare2($a, $b) {
    $first = strtolower($a['sort_value']);
    $second = strtolower($b['sort_value']);
    return strcmp($first, $second);
}

function array_merge_product($list1, $list2) {
    $list_key1 = array_keys($list1);
    $list_key2 = array_keys($list2);
    $all_keys = array_merge($list_key1, $list_key2);
    $ret = array();
    foreach ($all_keys as $key => $value) {
        $content1 = array();
        $content2 = array();
        if (isset($list1[$value])) {
            $content1 = $list1[$value];
        }
        if (isset($list2[$value])) {
            $content2 = $list2[$value];
        }
...
function get_call_api($apiKey, $pageID, $baseurl, $access_token, $iteration, $altapi) {
    if (!empty($altapi)) {
        $urlApi = $altapi;
    } else {
        $urlApi = $baseurl . '/properties?api_key=' . $apiKey . $pageID . $iteration;
