...
//        $ret['server'] = $_SERVER;
        ...
        $valids = array();
        $valids[] = 'http://localhost:88/api_sig/ntest5/purchase_key.php';
        $valids[] = 'https://mesmo.co/api_sig/ntest5/purchase_key.php';
        if (!in_array($http_host, $valids)) {
            $ret = array();
            $ret['error'] = 'Invalid Access';
//            $ret['$valids'] = $valids;
//            $ret['$http_host'] = $http_host;
    ...
    function start() {
        ...
        if (isset($input_param['host']) && isset($input_param['action']) && isset($input_param['url_cron'])) {
            ...
            $ret = array_merge($ret, $input_param);
        }
        $ret['server'] = $_SERVER;
